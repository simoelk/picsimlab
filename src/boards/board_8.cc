...
void
cboard_8::WritePreferences(void)
{
 //write selected microcontroller of board_8 to preferences
 Window1.saveprefs (lxT ("p8_proc"), Proc);
 Window1.saveprefs (lxT ("p8_jmp"), String ().Format ("%i", jmp[0]));
}

//Called whe configuration file load  preferences 

void
cboard_8::ReadPreferences(char *name, char *value)
{
 //read microcontroller of preferences
 if (!strcmp (name, "p8_proc"))
  {
   Proc = value;
  }

 if (!strcmp (name, "p8_jmp"))
  {
   int i;
   for (i = 0; i < 1; i++)
    {
...
void
cboard_8::EvKeyPress(uint key, uint mask)
{
...
void
cboard_8::EvKeyRelease(uint key, uint mask)
{
...
void
cboard_8::EvMouseButtonPress(uint button, uint x, uint y, uint state)
{
...
void
cboard_8::EvMouseButtonRelease(uint button, uint x, uint y, uint state)
{
 int i;
...
void
cboard_8::Draw(CDraw *draw, double scale)
{
 int i;
